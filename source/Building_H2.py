...
for train_ix, test_ix in cv_outer.split(x):
    ...
    # Define the model
    # max_depth and bootstrap are fixed from the most frequent one of the best_parameters
    model = RandomForestClassifier(min_samples_split = 10, min_samples_leaf = 1, bootstrap= True, random_state=42)
    
    # Define search space based on the frequency from best_parameteres
    # Total 6 combination
    space = dict()
...
print("Average accuracy of H2_FD: ",np.mean(accuracies))
print("Average precision of H2_FD: ",np.mean(precisions))
print("Average recall of H2_FD: ", np.mean(recalls))


# Final model on raw dataset
...
print("Average accuracy of H2_UD: ",np.mean(accuracies_raw))
print("Average precision of H2_UD: ",np.mean(precisions_raw))
print("Average recall of H2_UD: ", np.mean(recalls_raw))